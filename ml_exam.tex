...
\section{Q2.5 Write an \( L^2 \)-regularized minibatch SGD algorithm for training a linear regression model, including the explicit formulas (i.e, formulas you would need to code it with numpy) of the loss function and its gradient. [10]}
...
\section{Q3.2 Explain what it means for a dataset to be linearly separable. Give an example of a simple 2D dataset that is *not* linearly separable and explain why the perceptron algorithm would fail on it. [10]}
TODO
...
\section{Q3.8 Write down an $L^2$-regularized minibatch SGD algorithm for training a binary logistic regression model, including the explicit formulas (i.e., formulas you would need to code it in numpy) of the loss function and its gradient. [20]}
...
Let $\boldsymbol z \in \mathbb{R}^K$ be the logits and
\[
\boldsymbol y = \operatorname{softmax}(\boldsymbol z), 
\qquad
y_k = \frac{e^{z_k}}{\sum_{j=1}^K e^{z_j}}.
\]
In training, the negative log-likelihood (cross-entropy for a one-hot target) for true class $t$ is
\[
L(\boldsymbol z) = -\log y_t = -\log\left(\operatorname{softmax}(\boldsymbol z)_t\right).
\]

\subsection*{1) Derivative computation}

We use the well-known Jacobian of softmax:
\[
...
First,
\[
\frac{\partial L}{\partial y_k} = -\frac{1}{y_t}\,\delta_{kt}.
\]
By the chain rule,
\[
\frac{\partial L}{\partial z_i}
= \sum_{k=1}^K \frac{\partial L}{\partial y_k}\frac{\partial y_k}{\partial z_i}
= -\frac{1}{y_t}\frac{\partial y_t}{\partial z_i}.
\]
Substitute the softmax derivative for $k=t$:
\[
\frac{\partial y_t}{\partial z_i} = y_t(\delta_{ti} - y_i).
\]
Therefore,
\[
\frac{\partial L}{\partial z_i}
= -\frac{1}{y_t}\, y_t(\delta_{ti} - y_i)
= y_i - \delta_{ti}.
\]

\paragraph{Final result (componentwise).}
\[
\boxed{\;\frac{\partial}{\partial z_i}\left(-\log \operatorname{softmax}(\boldsymbol z)_t\right) = y_i - \mathbb{I}[i=t]\;}
\]

\paragraph{Vector form.}
Let $\boldsymbol t$ be the one-hot target vector, i.e.\ $t_i=\mathbb{I}[i=t]$. Then
\[
\boxed{\;\nabla_{\boldsymbol z} L = \boldsymbol y - \boldsymbol t.\;}
\]
(More generally, for a target distribution $\boldsymbol t$ in cross-entropy
$L=-\sum_k t_k\log y_k$, the same result holds: $\nabla_{\boldsymbol z}L=\boldsymbol y-\boldsymbol t$.)

\subsection*{2) How it is used when training an MLP}

Consider an MLP whose last (output) layer produces logits
\[
\boldsymbol z = (W^{(y)})^\top \boldsymbol h + \boldsymbol b^{(y)},
\qquad
\boldsymbol y=\operatorname{softmax}(\boldsymbol z),
\]
where $\boldsymbol h$ are hidden activations. The quantity
\[
\boldsymbol\delta^{(y)} \;\stackrel{\text{def}}{=}\; \frac{\partial L}{\partial \boldsymbol z}
= \boldsymbol y - \boldsymbol t
\]
is the \emph{output error signal} that starts backpropagation.

Using $\boldsymbol\delta^{(y)}$, the gradients of the output-layer parameters are
\[
\frac{\partial L}{\partial W^{(y)}} = \boldsymbol h\,(\boldsymbol\delta^{(y)})^\top,
\qquad
\frac{\partial L}{\partial \boldsymbol b^{(y)}} = \boldsymbol\delta^{(y)}.
\]
Then the gradient is propagated to the previous layer:
\[
\frac{\partial L}{\partial \boldsymbol h}
= W^{(y)}\boldsymbol\delta^{(y)}.
\]
If the hidden pre-activation is $\boldsymbol h^{(\mathrm{in})}$ and $\boldsymbol h=f(\boldsymbol h^{(\mathrm{in})})$, we obtain
\[
\boldsymbol\delta^{(h)} = \frac{\partial L}{\partial \boldsymbol h^{(\mathrm{in})}}
= \left(\frac{\partial L}{\partial \boldsymbol h}\right)\odot f'(\boldsymbol h^{(\mathrm{in})}),
\]
and analogously compute $\partial L/\partial W^{(h)}$ and $\partial L/\partial \boldsymbol b^{(h)}$.

Finally, SGD (or a variant) updates parameters using these gradients, e.g.
$W \leftarrow W - \eta \,\partial L/\partial W$.

\paragraph{Practical note (stability).}
In implementations, one typically computes \emph{log-softmax + NLL} together; the combined gradient
$\boldsymbol y-\boldsymbol t$ avoids forming unstable intermediate terms such as $\log(\cdot)$ of very small probabilities.
...
\subsection*{Numeric Instabilities in Naive Bayes Classifier}

\subsubsection*{1. Gaussian Naive Bayes (for continuous data):}

In Gaussian Naive Bayes, we assume that the features (or variables) follow a normal (Gaussian) distribution. The probability density function of the Gaussian is given by:

\[
P(x_i | y) = \frac{1}{\sqrt{2\pi \sigma_y^2}} \exp\left(-\frac{(x_i - \mu_y)^2}{2\sigma_y^2}\right)
\]

Where \( \mu_y \) and \( \sigma_y^2 \) are the mean and variance of the features for each class \( y \).

\textbf{Issue:}
\begin{itemize}
    \item \textit{High probability density:} If the variance \( \sigma_y^2 \) is very small (i.e., the feature values are very close to the mean), the probability density can become excessively high, potentially leading to numeric instability.
\end{itemize}

\textbf{Solutions:}
\begin{itemize}
    \item \textit{Regularization of variance:} Add a small constant to the variance to avoid dividing by a very small value, which helps prevent excessively large probability densities. This is typically done by adding a smoothing term:
    \[
    \sigma_y^2 \rightarrow \sigma_y^2 + \epsilon
    \]
    where \( \epsilon \) is a small positive constant, such as \( 10^{-10} \).
    
    \item \textit{Logarithmic transformation:} In practice, the Naive Bayes classifier often works with the logarithm of the probabilities (log-likelihoods) to avoid underflow when multiplying many small probabilities. For Gaussian Naive Bayes, you can compute:
    \[
    \log P(x_i | y) = -\frac{1}{2} \log(2\pi \sigma_y^2) - \frac{(x_i - \mu_y)^2}{2\sigma_y^2}
    \]
    This approach helps in both improving numerical stability and handling very small or very large probability values more effectively.
\end{itemize}

\subsubsection*{2. Bernoulli Naive Bayes (for binary features):}

In Bernoulli Naive Bayes, the features are binary (i.e., they can take values 0 or 1), and the likelihood of a feature given a class is modeled as a Bernoulli distribution:

\[
P(x_i | y) = p_i^{x_i} (1 - p_i)^{(1 - x_i)}
\]

Where \( p_i = P(x_i = 1 | y) \) is the probability of the feature being 1 for class \( y \).

\textbf{Issue:}
\begin{itemize}
    \item \textit{Zero probabilities:} If any feature has zero probability (e.g., \( P(x_i = 1 | y) = 0 \) for some feature \( x_i \) in class \( y \)), the entire likelihood for that class will be zero, which can cause issues when computing the posterior.
\end{itemize}

\textbf{Solutions:}
\begin{itemize}
    \item \textit{Laplace smoothing:} Add a small constant (typically 1) to the numerator and adjust the denominator accordingly to avoid zero probabilities:
    \[
    P(x_i = 1 | y) = \frac{ \sum_{j} x_{ij} + 1}{N_y + 2}
    \]
    where \( \sum_{j} x_{ij} \) is the number of times feature \( x_i \) is 1 in class \( y \), and \( N_y \) is the total number of samples in class \( y \). This technique is known as \textbf{Laplace smoothing} or \textbf{additive smoothing}.
    
    \item \textit{Logarithmic transformation:} As with Gaussian Naive Bayes, using the logarithm of the probabilities can help with numeric stability. For Bernoulli Naive Bayes, you would compute:
    \[
    \log P(x_i | y) = x_i \log p_i + (1 - x_i) \log (1 - p_i)
    \]
    This avoids underflow when multiplying probabilities and helps manage very small or zero probabilities.
\end{itemize}

\subsubsection*{3. General Measures for Both Types of Naive Bayes:}

\begin{itemize}
    \item \textit{Avoid direct multiplication of probabilities:} Instead of multiplying the probabilities directly (which can lead to very small numbers), work with the logarithms of probabilities. Since logarithms turn products into sums, this is computationally more stable and avoids overflow or underflow problems.
    
    \item \textit{Use of floating-point precision:} Ensure that the computations are done with sufficient floating-point precision (e.g., using double precision instead of single precision) to avoid rounding errors that may exacerbate numeric instability.
\end{itemize}

\subsection*{Summary of Measures:}

\textbf{1. Gaussian Naive Bayes:}
\begin{itemize}
    \item Add a small constant \( \epsilon \) to the variance to avoid very small values.
    \item Use logarithms of the likelihoods to handle small numbers more effectively.
\end{itemize}

\textbf{2. Bernoulli Naive Bayes:}
\begin{itemize}
    \item Apply Laplace smoothing (additive smoothing) to avoid zero probabilities.
    \item Use logarithms of the likelihoods to handle extreme values and avoid multiplication of zero probabilities.
\end{itemize}

By implementing these techniques, you can greatly reduce numeric instability and ensure that the Naive Bayes classifier performs reliably, even in challenging scenarios.


\section{Q7.7 What is the difference between discriminative and (classical) generative models? [5]}

The key difference between \textbf{discriminative} and \textbf{generative} models lies in how they model the data and the task they aim to solve:

Discriminative models focus on directly modeling the decision boundary between classes. They estimate the probability of the class given the data, \( P(y | x) \), where \( x \) is the input and \( y \) is the output. These models try to distinguish between different classes based on the features, without making assumptions about how the data is generated.

\begin{itemize}
    \item Example: \textbf{Logistic Regression}, \textbf{Support Vector Machines (SVMs)}, and \textbf{Neural Networks}.
    \item They are typically more \textit{data-efficient} for classification tasks because they focus on the conditional probability.
\end{itemize}

Generative models, on the other hand, model how the data is generated by learning the joint probability \( P(x, y) \). They aim to understand the underlying distribution of the features \( x \) for each class \( y \), and then use Bayes' theorem to compute \( P(y | x) \). In other words, they model how the data looks within each class and can generate new samples.

\begin{itemize}
    \item Example: \textbf{Naive Bayes}, \textbf{Gaussian Mixture Models}, and \textbf{Hidden Markov Models}.
    \item They are useful when you want to model the underlying data distribution, and they can be used for tasks like generating new data or handling missing data.
\end{itemize}

\begin{itemize}
    \item \textbf{Discriminative models} model \( P(y | x) \) directly (focused on classification).
    \item \textbf{Generative models} model \( P(x, y) \) (focus on modeling the data generation process).
\end{itemize}
...
TODO: "Name examples of tasks. What data do you need besides the model predictions and the targets?"
...
In classification, the \textbf{CART} algorithm builds a decision tree by recursively splitting the dataset into subsets. At each node, the algorithm chooses the best feature and corresponding threshold to split the data into two groups. The goal is to make the classes in the resulting subsets as pure as possible.

The objective is to minimize the \textbf{Gini impurity} at each node.

The \textbf{Gini impurity} measures the "impurity" of a dataset. For a dataset \( D \) with \( C \) classes, it is defined as:
\[
Gini(D) = 1 - \sum_{i=1}^{C} p_i^2
\]
where:
\begin{itemize}
    \item \( p_i \) is the proportion of instances of class \( i \) in the dataset \( D \),
    \item \( C \) is the number of classes.
\end{itemize}

The Gini impurity takes values between 0 (perfect purity) and 1 (maximum impurity). The lower the Gini impurity, the purer the node.

\begin{enumerate}
    \item \textbf{Starting at the Root Node:} 
    The algorithm calculates the Gini impurity of the entire dataset at the root node.
    
    \item \textbf{Splitting:} 
    The algorithm evaluates all possible splits based on different features and thresholds. For each possible split, the Gini impurity is computed for the resulting child nodes (two groups). The split that results in the lowest total Gini impurity across the child nodes is chosen.
    
    \item \textbf{Recursion:} 
    The process repeats recursively for each child node. Each time, the algorithm selects the best feature and threshold that minimizes the Gini impurity.
    
    \item \textbf{Stopping Criteria:} 
    The tree-building stops when a stopping criterion is met, such as a maximum depth, a minimum number of samples in a node, or when further splitting does not reduce the impurity significantly.
\end{enumerate}

The \textbf{loss function} in this context is related to the \textbf{Gini impurity}. The objective of the CART algorithm is to minimize the loss function at each step by choosing the split that leads to the lowest Gini impurity for the child nodes. This ensures that the data in each node becomes purer, meaning the class distribution in each node is as homogenous as possible.

In simpler terms:
\begin{itemize}
    \item \textbf{Gini impurity} quantifies the "impurity" or mixedness of the classes at each node.
    \item \textbf{Minimizing the Gini impurity} at each node results in a decision tree where the leaves are as pure as possible, leading to better classification performance.
\end{itemize}

This process is what allows the CART algorithm to construct a decision tree that is both effective and interpretable.

Instead of Gini impurity, the \textbf{entropy criterion} can also be used, where the goal is to minimize the entropy at each node. The entropy criterion is defined as: 
\[
H(T) = -\sum_{k=1}^{K} p_T(k) \log p_T(k)
\]
where \( p_T(k) \) is the proportion of samples of class \( k \) in node \( T \).

Also, for regression tasks, the \textbf{mean squared error (MSE)} can be used as the loss function to minimize the variance of the target variable at each node.
MSE is defined as:
\[
\text{MSE}(T) = \frac{1}{|I_T|} \sum_{i \in I_T} (t_i - \bar{t}_T)^2
\]
where \( t_i \) is the target value of the \( i \)-th sample, \( \bar{t}_T \) is the average target value in node \( T \), and \( I_T \) is the set of indices of samples in node \( T \).
...
TODO: This answers $K$-class, but binary is just $K=2$...
...
\subsection*{Second-Order Optimization using Newton's Method}

Second-order optimization methods, such as \textbf{Newton's method}, leverage information from the \textbf{second derivative} (the Hessian matrix in the case of multivariate problems) to optimize a function. The intuition behind this approach is that it uses both the \textbf{gradient} (first derivative) and the \textbf{curvature} (second derivative) of the objective function to make more informed updates during optimization, which can lead to faster convergence compared to first-order methods like gradient descent.

\subsubsection*{Newton’s Method}
Newton's method for optimization uses a \textbf{second-order Taylor expansion} around the current point \( x_k \) to approximate the objective function \( f(x) \):
...
\begin{itemize}
    ...
    \item \( H(x_k) \) is the Hessian matrix (second derivative) of \( f(x) \) at \( x_k \).
\end{itemize}

The first term is the function value at the current point, the second term represents the linear approximation of the function using the gradient, and the third term captures the curvature of the function using the Hessian.

\subsubsection*{Intuition Behind Second-Order Optimization}
\begin{itemize}
    \item \textbf{Gradient (First-Order Information):} The gradient gives the direction of steepest ascent or descent in the function. It tells you how the function changes as you move in each direction but doesn’t provide information about the shape of the function.
    \item \textbf{Hessian (Second-Order Information):} The Hessian matrix provides information about the curvature of the function. It tells you whether the function is concave up (positive curvature) or concave down (negative curvature) in various directions. A large value in the Hessian suggests that the function is steep in that direction, and a small value suggests a flatter region.
    \item \textbf{Using Both Gradients and Curvature:} In Newton’s method, you use both the gradient and the Hessian to update your position in a way that accounts for the steepness and curvature of the function. If the Hessian indicates that the function is steep, the update step is smaller (so as not to overshoot). If the Hessian indicates that the function is relatively flat, the update step is larger.
\end{itemize}
...
where:
\begin{itemize}
    \item \( H(x_k)^{-1} \) is the inverse of the Hessian matrix,
    \item \( \nabla f(x_k) \) is the gradient at the current point \( x_k \).
\end{itemize}

By using the second-order information, Newton’s method often converges more quickly than gradient descent, especially near the optimum, because it takes into account both the slope and the curvature of the objective function.

\subsubsection*{In Practice}
\begin{itemize}
    \item \textbf{Faster Convergence:} Second-order methods like Newton’s method can converge more quickly than first-order methods, especially when the objective function has well-behaved curvature.
    \item \textbf{Computational Complexity:} However, the main drawback is that computing the Hessian matrix and its inverse can be computationally expensive, especially for high-dimensional problems. Thus, while the method is more efficient in terms of the number of iterations needed to converge, it may not always be feasible for very large-scale problems.
\end{itemize}

In summary, second-order optimization, through methods like Newton’s, improves upon first-order methods by using not just the gradient but also the curvature of the objective function, leading to more informed and typically faster updates, especially when the objective function behaves well near the optimum.
...
TODO: "Explain how the loss formulation relates to Taylor's expansions."
